#!/usr/bin/env julia
# Build libpureblas.<dlext> via juliac --trim (experimental). The resulting shared
...
# ── juliac driver: the JuliaC.jl package ───────────────────────────────────────────────────────────
# The driver is the registered package JuliaC.jl (JuliaLang/JuliaC.jl), pinned in `juliac/toolenv`.
# Julia ships no driver of its own: `share/julia/test/trimming/juliac-{buildscript,trim-base,
# trim-stdlib}.jl` are test fixtures, not an entry point. The driver runs in its own environment, so
# the package being built is named explicitly with `--project=$ROOT`.
...
# And `cgemvn_nc_big`: the complex gemv-N column-panel width for the A > L3/2 tier is a Measure-tier
# OncePerProcess. `zgemv_64_` is @ccallable, so unpinned the .so's first complex gemv would allocate an
# L3-sized matrix and time three panel widths before answering. Value 8 = _vwidth(F64), the width the
# on-host tuner selects on Zen4 and the value Zen3 keeps by incumbency; it is also the middle candidate,
# so a host whose optimum sits either side loses only the margin, never the ~15% the old NC=4 cost at
# A≈L3. A trimmed .so is ONE binary that must be reasonable on any host — same reasoning as
...
try
    ...
    if _prev_gvtd === nothing
        delete_preferences!(PUREBLAS_UUID, "gemvt_deep"; force = true)
    else
        set_preferences!(PUREBLAS_UUID, "gemvt_deep" => _prev_gvtd; force = true)
    end
    if _prev_gvu === nothing
        delete_preferences!(PUREBLAS_UUID, "gemvt_u"; force = true)
    else
        set_preferences!(PUREBLAS_UUID, "gemvt_u" => _prev_gvu; force = true)
    end
    if _prev_gvpf === nothing
