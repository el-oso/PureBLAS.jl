...
for (p, T) in (("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "gesv_64_"))(n::Ptr{Int64}, nrhs::Ptr{Int64},
...
Base.@ccallable function sgesv_64_(n::Ptr{Int64}, nrhs::Ptr{Int64}, A::Ptr{Float32}, lda::Ptr{Int64},
        ipiv::Ptr{Int64}, B::Ptr{Float32}, ldb::Ptr{Int64}, info::Ptr{Int64})::Cvoid
    N = Int(unsafe_load(n)); R = Int(unsafe_load(nrhs))
    ...
    @inbounds for j in 1:R, i in 1:N; Bf[i, j] = Float64(Bm[i, j]); end
    ...
    @inbounds for j in 1:R, i in 1:N; Bm[i, j] = Float32(Bf[i, j]); end
    ipv = PtrVector(ipiv, N); @inbounds for i in 1:N; ipv[i] = ip[i]; end
    unsafe_store!(info, Int64(inf)); return
end
...
for (p, T) in (("s", Float32), ("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "posv_64_"))(uplo::Ptr{UInt8}, n::Ptr{Int64},
...
for (p, T) in (("s", Float32), ("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "lacpy_64_"))(uplo::Ptr{UInt8}, m::Ptr{Int64},
            n::Ptr{Int64}, A::Ptr{$T}, lda::Ptr{Int64}, B::Ptr{$T}, ldb::Ptr{Int64}, lu::Clong)::Cvoid
        ul = _cabi_char(uplo)
        M = Int(unsafe_load(m)); N = Int(unsafe_load(n))
        Am = PtrMatrix(A, M, N, Int(unsafe_load(lda)))
        Bm = PtrMatrix(B, M, N, Int(unsafe_load(ldb)))
        if ul == 'U'
            @inbounds for j in 1:N, i in 1:min(j, M); Bm[i, j] = Am[i, j]; end
        elseif ul == 'L'
            @inbounds for j in 1:N, i in j:M; Bm[i, j] = Am[i, j]; end
        else
            @inbounds for j in 1:N, i in 1:M; Bm[i, j] = Am[i, j]; end
        end
        return
    end
end
...
for (p, T) in (("s", Float32), ("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "larfg_64_"))(n::Ptr{Int64}, alpha::Ptr{$T}, x::Ptr{$T},
            incx::Ptr{Int64}, tau::Ptr{$T})::Cvoid
        N = Int(unsafe_load(n))
        if N <= 1
        ...
        @inbounds for j in 2:N; v[j] = unsafe_load(x, 1 + (j - 2) * ix); end
        β, τ = _larfg!(v)
        unsafe_store!(alpha, $T(β))
        @inbounds for j in 2:N; unsafe_store!(x, v[j], 1 + (j - 2) * ix); end
        unsafe_store!(tau, τ)
        return
    end
end
...
for (p, T) in (("s", Float32), ("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "larf_64_"))(side::Ptr{UInt8}, m::Ptr{Int64}, n::Ptr{Int64},
        ...
        @inbounds for j in 1:l; vv[j] = unsafe_load(v, 1 + (j - 1) * iv); end
        if sd == 'L'
            _house_left!(Cm, vv, τ)
        else
            _larf_right!(Cm, vv, τ)
...
for (p, T, Tr) in (("s", Float32, Float32), ("d", Float64, Float64),
    ...
    @eval Base.@ccallable function $(Symbol(p, "gebak_64_"))(job::Ptr{UInt8}, side::Ptr{UInt8},
            n::Ptr{Int64}, ilo::Ptr{Int64}, ihi::Ptr{Int64}, scale::Ptr{$Tr}, m::Ptr{Int64}, V::Ptr{$T},
            ldv::Ptr{Int64}, info::Ptr{Int64}, lj::Clong, ls::Clong)::Cvoid
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "hseqr_64_"))(job::Ptr{UInt8}, compz::Ptr{UInt8},
        ...
        inf = hseqr!(_cabi_char(job), _cabi_char(compz), Hm, Int(unsafe_load(ilo)), Int(unsafe_load(ihi)),
            wtmp, Zm)
        wrp = PtrVector(wr, N); wip = PtrVector(wi, N)
        @inbounds for i in 1:N; wrp[i] = real(wtmp[i]); wip[i] = imag(wtmp[i]); end
        unsafe_store!(info, Int64(inf)); return
    end
end
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "trevc_64_"))(side::Ptr{UInt8}, howmny::Ptr{UInt8},
...
for (p, T, R) in (("c", ComplexF32, Float32), ("z", ComplexF64, Float64))
    @eval Base.@ccallable function $(Symbol(p, "trevc_64_"))(side::Ptr{UInt8}, howmny::Ptr{UInt8},
            ...
            info::Ptr{Int64}, ls::Clong, lh::Clong)::Cvoid
        sd = _cabi_char(side); hn = _cabi_char(howmny)
        if sd != 'R' || (hn != 'A' && hn != 'B')
            unsafe_store!(info, Int64(-1)); return
        end
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "sytrd_64_"))(uplo::Ptr{UInt8}, n::Ptr{Int64}, A::Ptr{$T},
...
for (p, T, R) in (("c", ComplexF32, Float32), ("z", ComplexF64, Float64))
    @eval Base.@ccallable function $(Symbol(p, "hetrd_64_"))(uplo::Ptr{UInt8}, n::Ptr{Int64}, A::Ptr{$T},
...
# len_uplo)` — 1 char.
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "orgtr_64_"))(uplo::Ptr{UInt8}, n::Ptr{Int64}, A::Ptr{$T},
        ...
        Q = orgtr!('L', Am, tm)
        @inbounds for j in 1:N, i in 1:N; Am[i, j] = Q[i, j]; end
        unsafe_store!(info, Int64(0)); return
    end
end
for (p, T) in (("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "ungtr_64_"))(uplo::Ptr{UInt8}, n::Ptr{Int64}, A::Ptr{$T},
            lda::Ptr{Int64}, tau::Ptr{$T}, work::Ptr{$T}, lwork::Ptr{Int64}, info::Ptr{Int64},
            lu::Clong)::Cvoid
        if unsafe_load(lwork) == Int64(-1)
            unsafe_store!(work, one($T)); unsafe_store!(info, Int64(0)); return
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "ormtr_64_"))(side::Ptr{UInt8}, uplo::Ptr{UInt8},
            ...
            ls::Clong, lu::Clong, lt::Clong)::Cvoid
        if unsafe_load(lwork) == Int64(-1)
            unsafe_store!(work, one($T)); unsafe_store!(info, Int64(0)); return
        end
...
for (p, T) in (("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "unmtr_64_"))(side::Ptr{UInt8}, uplo::Ptr{UInt8},
            trans::Ptr{UInt8}, mC::Ptr{Int64}, nC::Ptr{Int64}, A::Ptr{$T}, lda::Ptr{Int64}, tau::Ptr{$T},
            C::Ptr{$T}, ldc::Ptr{Int64}, work::Ptr{$T}, lwork::Ptr{Int64}, info::Ptr{Int64},
            ls::Clong, lu::Clong, lt::Clong)::Cvoid
        if unsafe_load(lwork) == Int64(-1)
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "orgqr_64_"))(m::Ptr{Int64}, n::Ptr{Int64}, k::Ptr{Int64},
...
for (p, T) in (("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "ungqr_64_"))(m::Ptr{Int64}, n::Ptr{Int64}, k::Ptr{Int64},
            A::Ptr{$T}, lda::Ptr{Int64}, tau::Ptr{$T}, work::Ptr{$T}, lwork::Ptr{Int64},
            info::Ptr{Int64})::Cvoid
        if unsafe_load(lwork) == Int64(-1)
            unsafe_store!(work, one($T)); unsafe_store!(info, Int64(0)); return
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "ormqr_64_"))(side::Ptr{UInt8}, trans::Ptr{UInt8},
            m::Ptr{Int64}, n::Ptr{Int64}, k::Ptr{Int64}, A::Ptr{$T}, lda::Ptr{Int64}, tau::Ptr{$T},
            C::Ptr{$T}, ldc::Ptr{Int64}, work::Ptr{$T}, lwork::Ptr{Int64}, info::Ptr{Int64},
            ls::Clong, lt::Clong)::Cvoid
        if unsafe_load(lwork) == Int64(-1)
            unsafe_store!(work, one($T)); unsafe_store!(info, Int64(0)); return
        end
...
for (p, T) in (("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "unmqr_64_"))(side::Ptr{UInt8}, trans::Ptr{UInt8},
            m::Ptr{Int64}, n::Ptr{Int64}, k::Ptr{Int64}, A::Ptr{$T}, lda::Ptr{Int64}, tau::Ptr{$T},
            C::Ptr{$T}, ldc::Ptr{Int64}, work::Ptr{$T}, lwork::Ptr{Int64}, info::Ptr{Int64},
            ls::Clong, lt::Clong)::Cvoid
        if unsafe_load(lwork) == Int64(-1)
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "ormhr_64_"))(side::Ptr{UInt8}, trans::Ptr{UInt8},
            ...
            info::Ptr{Int64}, ls::Clong, lt::Clong)::Cvoid
        if unsafe_load(lwork) == Int64(-1)
            unsafe_store!(work, one($T)); unsafe_store!(info, Int64(0)); return
        end
        sd = _cabi_char(side); tr = _cabi_char(trans)
...
for (p, T) in (("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "unmhr_64_"))(side::Ptr{UInt8}, trans::Ptr{UInt8},
            mC::Ptr{Int64}, nC::Ptr{Int64}, ilo::Ptr{Int64}, ihi::Ptr{Int64}, A::Ptr{$T}, lda::Ptr{Int64},
            tau::Ptr{$T}, C::Ptr{$T}, ldc::Ptr{Int64}, work::Ptr{$T}, lwork::Ptr{Int64},
            info::Ptr{Int64}, ls::Clong, lt::Clong)::Cvoid
        if unsafe_load(lwork) == Int64(-1)
...
# info)` — 0 chars.
Base.@ccallable function dgebrd_64_(m::Ptr{Int64}, n::Ptr{Int64}, A::Ptr{Float64}, lda::Ptr{Int64},
...
Base.@ccallable function sgebrd_64_(m::Ptr{Int64}, n::Ptr{Int64}, A::Ptr{Float32}, lda::Ptr{Int64},
    ...
    dm = PtrVector(d, k); em = PtrVector(e, k)
    tqm = PtrVector(tauq, k); tpm = PtrVector(taup, k)
    @inbounds for i in 1:k; dm[i] = Float32(df[i]); em[i] = Float32(ef[i]); tqm[i] = Float32(tqf[i]); tpm[i] = Float32(tpf[i]); end
    unsafe_store!(info, Int64(0)); return
end
for (p, T, R) in (("c", ComplexF32, Float32), ("z", ComplexF64, Float64))
    @eval Base.@ccallable function $(Symbol(p, "gebrd_64_"))(m::Ptr{Int64}, n::Ptr{Int64}, A::Ptr{$T},
...
@inline function _bdsqr_wrap!(uplo::Char, d::AbstractVector{Float64}, e::AbstractVector{Float64}, Uacc, Vtacc)
    ...
    if !isnothing(Vscr)
        @inbounds for i in 1:n, j in 1:size(Vtacc, 2); Vscr[j, i] = Vtacc[i, j]; end
    end
    if uplo == 'U'
        bdsqr!(d, e, Uacc, Vscr)
    else
    ...
    if !isnothing(Vscr)
        @inbounds for i in 1:n, j in 1:size(Vtacc, 2); Vtacc[i, j] = Vscr[j, i]; end
    end
    return
end
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "bdsqr_64_"))(uplo::Ptr{UInt8}, n::Ptr{Int64},
        ...
        @inbounds for i in 1:N; df[i] = Float64(dm[i]); end
        @inbounds for i in 1:max(N - 1, 0); ef[i] = Float64(em[i]); end
        ...
        NRU > 0 && (@inbounds for j in 1:N, i in 1:NRU; Uacc[i, j] = Float64(Um0[i, j]); end)
        NCVT > 0 && (@inbounds for j in 1:NCVT, i in 1:N; Vtf[i, j] = Float64(Vtm0[i, j]); end)
        _bdsqr_wrap!(ul, df, ef, Uacc, Vtf)
        NRU > 0 && (@inbounds for j in 1:N, i in 1:NRU; Um0[i, j] = $T(Uacc[i, j]); end)
        NCVT > 0 && (@inbounds for j in 1:NCVT, i in 1:N; Vtm0[i, j] = $T(Vtf[i, j]); end)
        @inbounds for i in 1:N; dm[i] = $T(df[i]); end
        unsafe_store!(info, Int64(0)); return
    end
end
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "bdsdc_64_"))(uplo::Ptr{UInt8}, compq::Ptr{UInt8},
        ...
        if cq == 'N'
            ...
            @inbounds for i in 1:N; Uacc[i, i] = 1.0; Vtacc[i, i] = 1.0; end
            _bdsqr_wrap!(ul, df, ef, Uacc, Vtacc)
            um = PtrMatrix(u, N, N, Int(unsafe_load(ldu)))
            vtm = PtrMatrix(vt, N, N, Int(unsafe_load(ldvt)))
            @inbounds for j in 1:N, i in 1:N; um[i, j] = $T(Uacc[i, j]); vtm[i, j] = $T(Vtacc[i, j]); end
        end
        @inbounds for i in 1:N; dm[i] = $T(df[i]); end
        unsafe_store!(info, Int64(0)); return
    end
