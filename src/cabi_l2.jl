...
for (sym, T, kern) in ((:ssymv_64_, Float32, :_symv!), (:dsymv_64_, Float64, :_symv!),
                       (:chemv_64_, ComplexF32, :_hemv!), (:zhemv_64_, ComplexF64, :_hemv!))
    @eval Base.@ccallable function $sym(uplo::Ptr{UInt8}, n::Ptr{Int64}, alpha::Ptr{$T},
            A::Ptr{$T}, lda::Ptr{Int64}, x::Ptr{$T}, incx::Ptr{Int64},
            beta::Ptr{$T}, y::Ptr{$T}, incy::Ptr{Int64}, len_uplo::Clong)::Cvoid
        nn = unsafe_load(n)
        Am = _cabi_mat(A, unsafe_load(lda), nn, nn)
        $kern(_cabi_char(uplo) == 'U', nn, unsafe_load(alpha), Am,
            x, unsafe_load(incx), unsafe_load(beta), y, unsafe_load(incy))
        return
    end
end
...
for (op, kern) in (("trmv", :_trmv!), ("trsv", :_trsv!))
    for (p, T) in (("s", Float32), ("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
        @eval Base.@ccallable function $(Symbol(p, op, "_64_"))(
                ...
                A::Ptr{$T}, lda::Ptr{Int64}, x::Ptr{$T}, incx::Ptr{Int64},
                len_u::Clong, len_t::Clong, len_d::Clong)::Cvoid
            nn = unsafe_load(n)
...
for (sym, T, kern) in ((:sspmv_64_, Float32, :_spmv!), (:dspmv_64_, Float64, :_spmv!),
                       (:chpmv_64_, ComplexF32, :_hpmv!), (:zhpmv_64_, ComplexF64, :_hpmv!))
    @eval Base.@ccallable function $sym(uplo::Ptr{UInt8}, n::Ptr{Int64}, alpha::Ptr{$T},
            AP::Ptr{$T}, x::Ptr{$T}, incx::Ptr{Int64},
            beta::Ptr{$T}, y::Ptr{$T}, incy::Ptr{Int64}, len_uplo::Clong)::Cvoid
        $kern(_cabi_char(uplo) == 'U', unsafe_load(n), unsafe_load(alpha), AP,
            x, unsafe_load(incx), unsafe_load(beta), y, unsafe_load(incy))
        return
    end
end
...
for (sym, T, AT, kern) in ((:sspr_64_, Float32, Float32, :_spr!), (:dspr_64_, Float64, Float64, :_spr!),
                           (:chpr_64_, ComplexF32, Float32, :_hpr!), (:zhpr_64_, ComplexF64, Float64, :_hpr!))
    @eval Base.@ccallable function $sym(uplo::Ptr{UInt8}, n::Ptr{Int64}, alpha::Ptr{$AT},
            x::Ptr{$T}, incx::Ptr{Int64}, AP::Ptr{$T}, len_uplo::Clong)::Cvoid
...
for (sym, T, kern) in ((:sspr2_64_, Float32, :_spr2!), (:dspr2_64_, Float64, :_spr2!),
                       (:chpr2_64_, ComplexF32, :_hpr2!), (:zhpr2_64_, ComplexF64, :_hpr2!))
    @eval Base.@ccallable function $sym(uplo::Ptr{UInt8}, n::Ptr{Int64}, alpha::Ptr{$T},
            x::Ptr{$T}, incx::Ptr{Int64}, y::Ptr{$T}, incy::Ptr{Int64},
            AP::Ptr{$T}, len_uplo::Clong)::Cvoid
        $kern(_cabi_char(uplo) == 'U', unsafe_load(n), unsafe_load(alpha),
...
for (p, T) in (("s", Float32), ("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "gbmv_64_"))(
        ...
        _gbmv!(tc != 'N', tc == 'C', mm, nn, klv, kuv, unsafe_load(alpha), ABm,
            x, unsafe_load(incx), unsafe_load(beta), y, unsafe_load(incy))
        return
    end
end
...
for (sym, T, kern) in ((:ssbmv_64_, Float32, :_sbmv!), (:dsbmv_64_, Float64, :_sbmv!),
                       (:chbmv_64_, ComplexF32, :_hbmv!), (:zhbmv_64_, ComplexF64, :_hbmv!))
    @eval Base.@ccallable function $sym(uplo::Ptr{UInt8}, n::Ptr{Int64}, k::Ptr{Int64},
            alpha::Ptr{$T}, AB::Ptr{$T}, ldab::Ptr{Int64}, x::Ptr{$T}, incx::Ptr{Int64},
            beta::Ptr{$T}, y::Ptr{$T}, incy::Ptr{Int64}, len_uplo::Clong)::Cvoid
        nn = unsafe_load(n); kv = unsafe_load(k)
        ABm = _cabi_mat(AB, unsafe_load(ldab), kv + 1, nn)         # band packed (k+1)×n
        $kern(_cabi_char(uplo) == 'U', nn, kv, unsafe_load(alpha), ABm,
            x, unsafe_load(incx), unsafe_load(beta), y, unsafe_load(incy))
        return
    end
end
