...
@inline function _laic1(
        job::Int, x::AbstractVector{T}, sest::R, w::AbstractVector{T},
        gamma::T
    ) where {T <: BlasFloat, R <: Real}
...
function gelsy!(
        A::AbstractMatrix{T}, B::AbstractMatrix{T}, jpvt::AbstractVector{<:Integer},
        rcond::Real
    ) where {T <: BlasFloat}
