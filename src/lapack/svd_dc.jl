...
@inline function _jac_left!(M::AbstractMatrix{Float64}, ra::Int, rb::Int, c::Float64, s::Float64)
    (c == 1.0 && s == 0.0) && return
    a0 = ra + 1; b0 = rb + 1
    return @inbounds for j in 1:size(M, 2)
        a = M[a0, j]; b = M[b0, j]
        M[a0, j] = c * a - s * b
        M[b0, j] = s * a + c * b
    end
...
@inline function _secular_eq(
        shift::Float64, mu::Float64, col0p::AbstractVector{Float64},
...
function _secular_secant(
        sec::F, sh::Float64, left::Float64, right::Float64,
        mu_cur::Float64, mu_prev::Float64, f_cur::Float64, f_prev::Float64
    ) where {F}
...
function _secular_root(
        col0p::AbstractVector{Float64}, diagp::AbstractVector{Float64}, o::Int,
...
function _compute_singular_vectors!(
    ...
    @inbounds for k in 0:(n - 1)
        ...
        if !isnothing(um)
            ...
            for li in 1:o
                um[rowidx[li], actual_k + 1] = vbuf[li] * ninv
            end
        end
        if !isnothing(vm)
            ...
            for li in 2:o
                vm[rowidx[li], actual_k + 1] = vbuf[li] * ninv
            end
        end
    end
...
function _compute_svd_of_m!(um, vm, diag, col0, outer_perm::Vector{Int}, n::Int, ws::_DCWork)
    ...
    _compute_singular_vectors!(
        um, vm, zhat, outer_perm, col_perm_inv, actual_n, shifts, mus, n, o,
        zhp, diagp, rowidx, ws.vbuf
    )
    @inbounds for idx in 0:(actual_n - 1)
        diag[idx + 1] = s[actual_n - idx - 1 + 1]
    end
    return @inbounds for idx in 0:(n - actual_n - 1)
...
function _combine_v!(V, vm, cv, k::Int, rem::Int, n::Int)
    ...
    @inbounds for jj in 1:n
        cv[k + 1, jj] = vk0 * vm[1, jj]
    end
    return @inbounds for jj in 1:n, ii in 1:n
        V[ii, jj] = cv[ii, jj]
    end
end
...
function _combine_u!(U, um, cu, k::Int, rem::Int, n::Int)
    ...
    return @inbounds for jj in 1:(n + 1), ii in 1:(n + 1)
        U[ii, jj] = cu[ii, jj]
    end
end
...
function _dc!(
        diag::AbstractVector{Float64}, subdiag::AbstractVector{Float64}, U, V, threshold::Int,
        ws::_DCWork
    )
    ...
    if n < threshold
        ...
        @inbounds for jj in 1:(n + 1), ii in 1:(n + 1)
            U[ii, jj] = ualloc[ii, jj]
        end
        return
    end
    mx = 0.0
    ...
    if mx == 0.0
        fill!(U, 0.0); @inbounds for i in 1:(n + 1)
            U[i, i] = 1.0
        end
        if !isnothing(V)
            fill!(V, 0.0); @inbounds for i in 1:n
                V[i, i] = 1.0
            end
        end
        return
