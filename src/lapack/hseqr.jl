...
function _zlahqr!(
        wantt::Bool, wantz::Bool, H::AbstractMatrix{T}, ilo::Int, ihi::Int,
        w::AbstractVector{T}, iloz::Int, ihiz::Int, Z::AbstractMatrix{T}
    ) where {T <: Complex}
    R = real(T)
    Z0 = zero(T); ONE = one(T)
    ...
    @inbounds for ii in (ilo + 1):ihi
        if imag(H[ii, ii - 1]) != RZERO
            ...
            for j in ii:jhi
                H[ii, j] *= sc
            end              # ZSCAL row ii, cols ii:jhi
            for r in jlo:min(jhi, ii + 1)
                H[r, ii] *= conj(sc)
            end  # ZSCAL col ii, rows jlo:min(jhi,ii+1)
            if wantz
                for r in iloz:ihiz
                    Z[r, ii] *= conj(sc)
                end
...
"""
function hseqr!(
        job::AbstractChar, compz::AbstractChar, H::AbstractMatrix{T}, ilo::Integer,
        ihi::Integer, w::AbstractVector, Z::AbstractMatrix{T}
    ) where {T <: Real}
    n = size(H, 1)
...
function hseqr!(
        job::AbstractChar, compz::AbstractChar, H::AbstractMatrix{T}, ilo::Integer,
        ihi::Integer, w::AbstractVector{T}, Z::AbstractMatrix{T}
    ) where {T <: Complex}
    n = size(H, 1)
    size(H, 2) == n || throw(DimensionMismatch("hseqr!: H must be square"))
    (job === 'E' || job === 'S') || throw(ArgumentError("hseqr!: job must be 'E' or 'S'"))
