...
@inline function _ladiv(a::R, b::R, c::R, d::R) where {R <: Real}
    ...
    if ab >= HALF * ov
        aa *= HALF; bb *= HALF; s *= TWO
    end
    if cd >= HALF * ov
        cc *= HALF; dd *= HALF; s *= HALF
    end
    if ab <= un * BS / ϵ
        aa *= be; bb *= be; s /= be
    end
...
@inline function _grot_rows!(M::AbstractMatrix{R}, i1::Int, i2::Int, jlo::Int, jhi::Int, c::R, s::R) where {R <: Real}
    return @inbounds for j in jlo:jhi
        t = M[i1, j]; u = M[i2, j]
        M[i1, j] = c * t + s * u
        M[i2, j] = c * u - s * t
    end
...
@inline function _grot_cols!(M::AbstractMatrix{R}, j1::Int, j2::Int, ilo::Int, ihi::Int, c::R, s::R) where {R <: Real}
    return @inbounds for i in ilo:ihi
        t = M[i, j1]; u = M[i, j2]
        M[i, j1] = c * t + s * u
        M[i, j2] = c * u - s * t
    end
...
"""
function gghrd!(
        compq::AbstractChar, compz::AbstractChar, A::AbstractMatrix{T}, B::AbstractMatrix{T},
        Q::AbstractMatrix{T}, Z::AbstractMatrix{T}; ilo::Integer = 1, ihi::Integer = size(A, 1)
    ) where {T <: Real}
    n = size(A, 1)
...
# ═══════════════════════════════════════════════════════════════════════════════════════════════════════
function _hgeqz!(
        wantt::Bool, wantq::Bool, wantz::Bool, H::AbstractMatrix{R}, T::AbstractMatrix{R},
...
"""
function hgeqz!(
        job::AbstractChar, compq::AbstractChar, compz::AbstractChar, H::AbstractMatrix{R},
        T::AbstractMatrix{R}, alpha::AbstractVector{<:Complex}, beta::AbstractVector{R},
        Q::AbstractMatrix{R}, Z::AbstractMatrix{R}; ilo::Integer = 1, ihi::Integer = size(H, 1)
    ) where {R <: Real}
    n = size(H, 1)
...
# ═══════════════════════════════════════════════════════════════════════════════════════════════════════
function _zhgeqz!(
        wantt::Bool, wantq::Bool, wantz::Bool, H::AbstractMatrix{C}, T::AbstractMatrix{C},
        ...
        @inbounds while jiter < maxit
            ...
            if route == 60
                ...
                if absb > safmin
                    ...
                    if ilschr
                        for i in ifrstm:(ilast - 1)
                            T[i, ilast] *= signbc
                        end
                        for i in ifrstm:ilast
                            H[i, ilast] *= signbc
                        end
                    else
...
function hgeqz!(
        job::AbstractChar, compq::AbstractChar, compz::AbstractChar, H::AbstractMatrix{C},
        T::AbstractMatrix{C}, alpha::AbstractVector{C}, beta::AbstractVector{C},
        Q::AbstractMatrix{C}, Z::AbstractMatrix{C}; ilo::Integer = 1, ihi::Integer = size(H, 1)
    ) where {C <: Complex}
    n = size(H, 1)
    wantt = job === 'S'; wantq = compq !== 'N'; wantz = compz !== 'N'
    _qz_init_qz!(compq, Q, n); _qz_init_qz!(compz, Z, n)
