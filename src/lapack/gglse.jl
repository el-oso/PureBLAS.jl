...
function gglse!(
    ...
    np = n - p                                           # y₁ length
    # Six borrows live across the whole driver. `ĉ` stays an m×1 MATRIX, not a vector: `_ggl_apply_Zh!`
    # wants a matrix, and taking one directly avoids a `reshape` — `reshape(::SubArray, m, 1)` routes to
    # Base._throw_dmrs, which `juliac --trim=safe` rejects. Matrix roles (`G` n×n, `Atil` m×n) take the
    # default exact ld, same criterion as gels!/gelsd!: `_wsgrow` already gave ld = row count at every
    # new high-water mark, so this reproduces today's stride and makes it independent of call history;
    # `_odd_ld` padding would be a new stride and is not taken without a measurement. None of the six
    # escapes — `x` is the caller's buffer and the returned residual is a scalar.
    @scope arn begin
        G = borrow!(arn, T, n, n)
        Atil = borrow!(arn, T, m, n)
        tauz = borrow!(arn, T, min(m, n))
        ĉ = borrow!(arn, T, m, 1)
        y = borrow!(arn, T, n)
        r = borrow!(arn, T, m)
