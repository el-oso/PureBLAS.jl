...
"""
function gecon!(
        normA::Real, A_lu::AbstractMatrix{T}, ipiv::AbstractVector{<:Integer};
        norm::Char = '1'
    ) where {T <: BlasFloat}
    n = size(A_lu, 1)
...
function pocon!(normA::Real, A_chol::AbstractMatrix{T}; uplo::Char = 'L', norm::Char = '1') where {T <: BlasFloat}
    ...
    solve! = uplo == 'L' ?
        (
            x -> (
                trsv!(A_chol, x; uplo = 'L', trans = 'N', diag = 'N');
                trsv!(A_chol, x; uplo = 'L', trans = 'C', diag = 'N'); x
            )
        ) :
        (
            x -> (
                trsv!(A_chol, x; uplo = 'U', trans = 'C', diag = 'N');
