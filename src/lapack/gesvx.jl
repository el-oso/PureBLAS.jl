...
function _laqge!(
        A::AbstractMatrix{T}, R::AbstractVector{Tr}, C::AbstractVector{Tr},
        rowcnd::Tr, colcnd::Tr, amax::Tr
    ) where {T, Tr <: Real}
    m, n = size(A)
...
function _gerfs!(
        trans::Char, A::AbstractMatrix{T}, AF::AbstractMatrix{T}, ipiv::AbstractVector{<:Integer},
        B::AbstractMatrix{T}, X::AbstractMatrix{T}, ferr::AbstractVector{Tr},
        berr::AbstractVector{Tr}, ainv_inf::Tr
    ) where {T, Tr <: Real}
...
function _lu_solve_vec!(
        trans::Char, AF::AbstractMatrix{T}, ipiv::AbstractVector{<:Integer},
        d::AbstractVector{T}
    ) where {T}
    n = length(d)
    if trans == 'N'
...
_anorm1(A) = (
    m = size(A, 1); n = size(A, 2); v = zero(real(eltype(A)));
    @inbounds for j in 1:n
        s = zero(real(eltype(A))); for i in 1:m
            s += abs(A[i, j])
        end; v = max(v, s)
    end; v
)
_anorm_inf(A) = (
    m = size(A, 1); n = size(A, 2); v = zero(real(eltype(A)));
    @inbounds for i in 1:m
        s = zero(real(eltype(A))); for j in 1:n
            s += abs(A[i, j])
        end; v = max(v, s)
    end; v
)
_amax(A) = (
    v = zero(real(eltype(A))); @inbounds for a in A
        v = max(v, abs(a))
    end; v
)
...
function gesvx!(
    ...
    umax = zero(Tr); @inbounds for j in 1:n, i in 1:j
        umax = max(umax, abs(AF[i, j]))
    end
    amx = _amax(A)
    ...
    if (notran && rowequ) || (!notran && colequ)
        f = notran ? R : C
        @inbounds for j in 1:nrhs, i in 1:n
            B[i, j] *= f[i]
        end
    end
