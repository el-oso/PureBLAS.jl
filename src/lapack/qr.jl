...
@inline function _qr_reflect_f64!(p::Ptr{Float64}, m::Int, col::Int, ld::Int)
    ...
    @inbounds while i + _CHOLW - 1 <= m
        x = vload(_CVF, _cvptr(p, i, col, ld)); acc = muladd(x, x, acc); i += _CHOLW
    end
    ...
    i = col + 1
    @inbounds while i + _CHOLW - 1 <= m
        b = _cvptr(p, i, col, ld); vstore(vload(_CVF, b) * vi, b); i += _CHOLW
    end
    @inbounds while i <= m
        unsafe_store!(p, unsafe_load(p, _clidx(i, col, ld)) * hi, _clidx(i, col, ld)); i += 1
    end
...
@inline function _qr_apply1_f64!(p::Ptr{Float64}, m::Int, col::Int, j::Int, ld::Int, tinv::Float64)
    return @inbounds begin
        ...
        while i + _CHOLW - 1 <= m
            bj = _cvptr(p, i, j, ld); vstore(muladd(vk, vload(_CVF, _cvptr(p, i, col, ld)), vload(_CVF, bj)), bj); i += _CHOLW
        end
        while i <= m
    ...
    GC.@preserve A begin
        ...
        @inbounds while col <= k
            ...
            begin
                ...
                if isinf(t1)
                    for j in (col + 2):n
                        _qr_apply1_f64!(p, m, col, j, ld, t0)
                    end
                else
                    g = unsafe_load(p, _clidx(col + 1, col, ld)); gacc = _CVF(0.0); i = col + 2   # g = v0ᵀv1
                    while i + _CHOLW - 1 <= m
                        gacc = muladd(vload(_CVF, _cvptr(p, i, col, ld)), vload(_CVF, _cvptr(p, i, col + 1, ld)), gacc); i += _CHOLW
                    end
                    g += sum(gacc)
                    while i <= m
                        g = muladd(unsafe_load(p, _clidx(i, col, ld)), unsafe_load(p, _clidx(i, col + 1, ld)), g); i += 1
                    end
                    ...
                    for j in (col + 2):n                                                      # FUSED rank-2 apply
                        ...
                        while i <= m
                            aj = unsafe_load(p, _clidx(i, j, ld)); d0 = muladd(unsafe_load(p, _clidx(i, col, ld)), aj, d0); d1 = muladd(unsafe_load(p, _clidx(i, col + 1, ld)), aj, d1); i += 1
                        end
                        ...
                        while i <= m
                            unsafe_store!(p, muladd(kk1, unsafe_load(p, _clidx(i, col + 1, ld)), muladd(kk0, unsafe_load(p, _clidx(i, col, ld)), unsafe_load(p, _clidx(i, j, ld)))), _clidx(i, j, ld)); i += 1
                        end
                    end
                end
                col += 2
...
@inline function _zqr_apply1!(p::Ptr{Complex{R}}, m::Int, col::Int, j::Int, ld::Int, csz::Int, tc::Complex{R}) where {R}
    ...
    return mt > 0 && _axpy_cmplx_simd!(mt, -real(twc), -imag(twc), vp, cjp)
end
    ...
    GC.@preserve A begin
        ...
        @inbounds while col <= k
            ...
            if col + 1 <= k && t0 != zero(T)
                ...
                if t1 == zero(T)
                    for j in (col + 2):n
                        _zqr_apply1!(p, m, col, j, ld, csz, conj(t0))
                    end
                else
                ...
                for j in (col + 1):n
                    t0 == zero(T) || _zqr_apply1!(p, m, col, j, ld, csz, conj(t0))
                end
                col += 1
            end
        end
