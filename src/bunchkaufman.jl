                ...
                if k < n
                    if herm
                        ...
                        for j in (k + 1):n               # rank-1 downdate of lower A(k+1:n,k+1:n)
                            wj = r1 * conj(A[j, k])
                            for i in j:n; A[i, j] -= A[i, k] * wj; end
                ...
                if k > 1
                    if herm
                        ...
                        for j in 1:(k - 1)                # rank-1 downdate on ORIGINAL column k
                            wj = r1 * conj(A[j, k])
                            for i in 1:j; A[i, j] -= A[i, k] * wj; end
                        end
                        for j in 1:(k - 1); A[j, k] *= r1; end   # then scale column (DSCAL after DSYR)
                        A[k, k] = real(A[k, k])
                    else
                        d11 = one(T) / A[k, k]
                        for j in 1:(k - 1)
    ...
    @inbounds begin
        ...
        while k <= n
            if ipiv[k] > 0                               # 1×1
                ...
                for j in 1:nrhs, i in (k + 1):n; B[i, j] -= A[i, k] * B[k, j]; end
                dk = herm ? real(A[k, k]) : A[k, k]
                for j in 1:nrhs; B[k, j] /= dk; end
                k += 1
            else                                         # 2×2, rows (k,k+1)
                kp = -ipiv[k]
        ...
        while k >= 1
            if ipiv[k] > 0                               # 1×1
                for j in 1:nrhs
                    s = zero(eltype(B))
                    for i in (k + 1):n; s += (herm ? conj(A[i, k]) : A[i, k]) * B[i, j]; end
                    B[k, j] -= s
                end
                kp = ipiv[k]
                kp != k && _bk_swap_rows!(B, k, kp)
    ...
    @inbounds begin
        ...
        while k >= 1
            if ipiv[k] > 0                               # 1×1
                kp = ipiv[k]
                kp != k && _bk_swap_rows!(B, k, kp)
                for j in 1:nrhs, i in 1:(k - 1); B[i, j] -= A[i, k] * B[k, j]; end
                dk = herm ? real(A[k, k]) : A[k, k]
                for j in 1:nrhs; B[k, j] /= dk; end
                k -= 1
            else                                         # 2×2, rows (k-1,k)
        ...
        while k <= n
            if ipiv[k] > 0                               # 1×1
                for j in 1:nrhs
                    s = zero(eltype(B))
                    for i in 1:(k - 1); s += (herm ? conj(A[i, k]) : A[i, k]) * B[i, j]; end
                    B[k, j] -= s
                end
                kp = ipiv[k]
                kp != k && _bk_swap_rows!(B, k, kp)
