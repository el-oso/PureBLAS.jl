...
# `_EXPINT[2]` shifts the cut and `_EXPINT[4]` witnesses that the unpacked side actually fired — both
...
@inline function _use_unpacked(m, n, k)
    u = max(m, n, k) <= _GEMM_UNPACK_MAX + @inbounds(_EXPINT[2])
    u && @inbounds(_EXPINT[4] = 1)
    return u
end
