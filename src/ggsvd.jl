...
function _ggs_qr_apply_left!(Ablk::AbstractMatrix{T}, tau::AbstractVector{T}, kk::Int,
...
function _ggs_formQ!(Qout::AbstractMatrix{T}, Ablk::AbstractMatrix{T}, tau::AbstractVector{T},
                     kk::Int) where {T}
    mm = size(Qout, 1)
    fill!(Qout, zero(T))
...
function _ggs_lags2(upper::Bool, a1::RT, a2::Complex{RT}, a3::RT,
                    b1::RT, b2::Complex{RT}, b3::RT) where {RT<:Real}
    T = Complex{RT}
...
function _ggs_ggsvp!(wantu::Bool, wantv::Bool, wantq::Bool, A::AbstractMatrix{T},
                     B::AbstractMatrix{T}, tola::Real, tolb::Real, U::AbstractMatrix{T},
                     V::AbstractMatrix{T}, Q::AbstractMatrix{T}) where {T}
    m, n = size(A)
    p = size(B, 1)
...
function _ggs_tgsja!(wantu::Bool, wantv::Bool, wantq::Bool, m::Int, p::Int, n::Int, k::Int, l::Int,
    ...
    @inbounds for _ in 1:maxit
        ...
        for i in 1:(l - 1), j in (i + 1):l
            ...
            k + j <= m && _ggs_rot!(view(A, k + j, (n - l + 1):n), view(A, k + i, (n - l + 1):n),
                                    csu, conj(snu))
...
"""
function ggsvd!(jobu::AbstractChar, jobv::AbstractChar, jobq::AbstractChar,
                A::AbstractMatrix{T}, B::AbstractMatrix{T}) where {T}
    (jobu == 'U' || jobu == 'N') ||
