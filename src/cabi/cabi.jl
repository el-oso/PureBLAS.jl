...
for (p, T) in (("s", Float32), ("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
    @eval begin
        Base.@ccallable function $(Symbol(p, "axpy_64_"))(
                n::Ptr{Int64}, a::Ptr{$T},
        ...
        Base.@ccallable function $(Symbol(p, "scal_64_"))(
                n::Ptr{Int64}, a::Ptr{$T},
                x::Ptr{$T}, incx::Ptr{Int64}
            )::Cvoid
            _scal!(unsafe_load(n), unsafe_load(a), x, unsafe_load(incx))
            return
        end
        Base.@ccallable function $(Symbol(p, "copy_64_"))(
                n::Ptr{Int64}, x::Ptr{$T},
        ...
        Base.@ccallable function $(Symbol(p, "swap_64_"))(
                n::Ptr{Int64}, x::Ptr{$T},
                incx::Ptr{Int64}, y::Ptr{$T}, incy::Ptr{Int64}
            )::Cvoid
...
for (p, T) in (("s", Float32), ("d", Float64))
    @eval Base.@ccallable function $(Symbol(p, "dot_64_"))(
            n::Ptr{Int64}, x::Ptr{$T},
            incx::Ptr{Int64}, y::Ptr{$T}, incy::Ptr{Int64}
        )::$T
...
for (sym, T, R) in (
        (:snrm2_64_, Float32, Float32), (:dnrm2_64_, Float64, Float64),
...
for (sym, T, R) in (
        (:sasum_64_, Float32, Float32), (:dasum_64_, Float64, Float64),
...
# iamax → Int64 index.
for (sym, T) in (
        (:isamax_64_, Float32), (:idamax_64_, Float64),
...
@inline function _gemm_cabi!(
        transa::Ptr{UInt8}, transb::Ptr{UInt8}, m::Int64, n::Int64, k::Int64,
        alpha::T, A::Ptr{T}, lda::Int64, B::Ptr{T}, ldb::Int64,
        beta::T, C::Ptr{T}, ldc::Int64
    ) where {T}
...
for (p, T) in (("s", Float32), ("d", Float64), ("c", ComplexF32), ("z", ComplexF64))
    @eval Base.@ccallable function $(Symbol(p, "gemm_64_"))(
            ...
            beta::Ptr{$T}, C::Ptr{$T}, ldc::Ptr{Int64}, len_ta::Clong, len_tb::Clong
        )::Cvoid
        _gemm_cabi!(
            transa, transb, unsafe_load(m), unsafe_load(n), unsafe_load(k),
