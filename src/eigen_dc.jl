...
function _secular_secant(sec::F, sh::T, left::T, right::T,
        mu_cur::T, mu_prev::T, f_cur::T, f_prev::T) where {T<:Real, F}
...
@inline function _secular_eq_eigen(shift::T, mu::T, w::AbstractVector{T}, dlambda::AbstractVector{T},
        rho::T, K::Int) where {T<:Real}
    res = one(T)
...
function _dc_eigen!(d::AbstractVector{T}, e::AbstractVector{T}, Z::AbstractMatrix{T},
        nb::Int = _STEDC_NB) where {T<:Real}
    n = length(d)
    if n == 0
        return
    end
    ...
    if n <= nb
        ...
        @inbounds for i in 1:n-1; ec[i] = e[i]; end
    ...
    z = Vector{T}(undef, n)
    @inbounds for j in 1:k; z[j] = Z1[k, j]; end
    @inbounds for j in 1:n2; z[k + j] = Z2[1, j]; end
    if rho_raw < 0
        @inbounds for j in k + 1:n; z[j] = -z[j]; end
    end
    invs2 = one(T) / sqrt(T(2))
    @inbounds for j in 1:n; z[j] *= invs2; end
    rho = T(2) * absr
    ...
    if K > 0
        ...
        wsq = zero(T)
        @inbounds for i in 1:K; wsq += w[i] * w[i]; end
        ...
        @inbounds for j in 1:K
            pos = survivor_idx[j]
            Dout[pos] = lam[j]
            for i in 1:n; Qout[i, pos] = R[i, j]; end
        end
    end
...
function _stedc!(d::AbstractVector{T}, e::AbstractVector{T}, Z::AbstractMatrix{T};
        nb::Int = _STEDC_NB) where {T<:Real}
    n = length(d)
    ...
    @inbounds while lo <= n
        ...
        if m == 1
            ...
            for i in lo:hi; orgnrm = max(orgnrm, abs(d[i])); end
            for i in lo:hi - 1; orgnrm = max(orgnrm, abs(e[i])); end
            if orgnrm != zero(T)
                for i in lo:hi; d[i] /= orgnrm; end
                for i in lo:hi - 1; e[i] /= orgnrm; end
            end
            _dc_eigen!(db, eb, Zb, nb)                           # solve the unit-scaled block (fills Zb)
            if orgnrm != zero(T)
                for i in lo:hi; d[i] *= orgnrm; end              # unscale eigenvalues (vectors are scale-free)
            end
        end
        lo = hi + 1
    end
