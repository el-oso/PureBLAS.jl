...
function _pttrs_core!(D::AbstractVector{<:Real}, E::AbstractVector{T},
                      Bm::AbstractMatrix{T}, upper::Bool) where {T <: Number}
    n = length(D)
    nrhs = size(Bm, 2)
...
function pttrs!(D::AbstractVector{<:Real}, E::AbstractVector{T},
                B::AbstractVecOrMat{T}; uplo::AbstractChar = 'L') where {T <: Number}
...
function ptsv!(D::AbstractVector{<:Real}, E::AbstractVector{T},
               B::AbstractVecOrMat{T}; uplo::AbstractChar = 'L') where {T <: Number}
    _, _, info = pttrf!(D, E)
