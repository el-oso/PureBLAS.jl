...
@inline function _sturm_negcount(d::AbstractVector{T}, e2::AbstractVector{T},
        i1::Int, i2::Int, x::T, pivmin::T) where {T<:Real}
    cnt = 0
    tmp = d[i1] - x
...
@inline function _sturm_bisect(d::AbstractVector{T}, e2::AbstractVector{T}, i1::Int, i2::Int,
        kloc::Int, low::T, high::T, pivmin::T, atoli::T, rtoli::T) where {T<:Real}
    @inbounds while true
...
"""
function stebz!(range::AbstractChar, order::AbstractChar, vl::T, vu::T, il::Integer, iu::Integer,
        abstol::Real, d::AbstractVector{T}, e::AbstractVector{T}) where {T<:Real}
...
function _dlagtf!(a::AbstractVector{T}, λ::T, b::AbstractVector{T}, c::AbstractVector{T},
        tol::T, d2::AbstractVector{T}, inn::AbstractVector{Int}) where {T<:Real}
    n = length(a)
    a[1] -= λ
    inn[n] = 0
    if n == 1
...
function _dlagts!(a::AbstractVector{T}, b::AbstractVector{T}, c::AbstractVector{T},
        d2::AbstractVector{T}, inn::AbstractVector{Int}, y::AbstractVector{T}, tol::T) where {T<:Real}
    n = length(a)
    epsm = eps(T) / 2                       # DLAMCH('E')
...
"""
function stein!(d::AbstractVector{T}, e::AbstractVector{T}, w::AbstractVector{T},
        iblock::AbstractVector{<:Integer}, isplit::AbstractVector{<:Integer}) where {T<:Real}
    n = length(d)
    m = length(w)
    ...
    @inbounds for nblk in 1:nblkmax
        ...
        while j <= m
            ...
            while true
                ...
                @simd for t in 1:bz; r1[t] *= scl; end
            ...
            @simd for t in 1:bz; r1[t] *= scl; end
            for t in 1:bz; Z[b1+t-1, j] = r1[t]; end
...
@inline function _dot(x::AbstractVector{T}, y::AbstractVector{T}) where {T<:Real}
    s = zero(T)
    @inbounds @simd for i in eachindex(x); s = muladd(x[i], y[i], s); end
    return s
end
@inline function _axpy!(a::T, x::AbstractVector{T}, y::AbstractVector{T}) where {T<:Real}
    @inbounds @simd for i in eachindex(x); y[i] = muladd(a, x[i], y[i]); end
    return y
end
@inline function _nrm2(x::AbstractVector{T}) where {T<:Real}
    s = zero(T)
    @inbounds @simd for i in eachindex(x); s = muladd(x[i], x[i], s); end
    return sqrt(s)
end
