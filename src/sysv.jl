...
function _sytri_lower!(A::AbstractMatrix{T}, ipiv::AbstractVector{<:Integer}, herm::Bool,
...
function _sytri_upper!(A::AbstractMatrix{T}, ipiv::AbstractVector{<:Integer}, herm::Bool,
        work::AbstractVector{T}) where {T}
    n = size(A, 1)
    Tr = real(T)
    @inbounds begin
