...
@inline function _corner_left!(C::AbstractMatrix{T}, v::AbstractVector{T}, τ::T) where {T}
    ...
    @inbounds for j in 1:nc
        w = C[len, j]
        for i in 1:len-1; w += conj(v[i]) * C[i, j]; end   # w = vᴴ·C[:,j]  (v[len]=1)
        w *= τ
        C[len, j] -= w
        for i in 1:len-1; C[i, j] -= v[i] * w; end          # C[:,j] −= τ·(vᴴC)·v
    end
    return C
end
...
@inline function _corner_right!(C::AbstractMatrix{T}, v::AbstractVector{T}, τ::T) where {T}
    ...
    @inbounds for i in 1:nr
        w = C[i, len]
        for j in 1:len-1; w += C[i, j] * v[j]; end          # w = (C·v)_i  (v[len]=1)
        w *= τ
        C[i, len] -= w
        for j in 1:len-1; C[i, j] -= w * conj(v[j]); end     # C[i,:] −= τ·w·vᴴ
    end
    return C
end
...
function orgql!(A::AbstractMatrix{T}, tau::AbstractVector{T}) where {T}
    ...
    @inbounds begin
        ...
        for i in 1:k
            ...
            for l in 1:s-1; A[l, c] *= -τ; end               # DSCAL(−τ) the essential
            A[s, c] = one(T) - τ
            for l in s+1:m; A[l, c] = zero(T); end
        end
    end
    return A
end
...
"""
function ormql!(side::Char, trans::Char, A::AbstractMatrix{T}, tau::AbstractVector{T},
...
function gerqf!(A::AbstractMatrix{T}, tau::AbstractVector{T}) where {T}
    ...
    @inbounds for i in k:-1:1
        ...
        for jj in 1:t; A[r, jj] = conj(A[r, jj]); end        # ZLACGV the row (no-op real)
        ...
        for jj in 1:t; A[r, jj] = conj(A[r, jj]); end        # ZLACGV back
    end
    return A
end
...
function orgrq!(A::AbstractMatrix{T}, tau::AbstractVector{T}) where {T}
    ...
    @inbounds begin
        if k < m                                             # init leading m−k rows to unit rows of I
            for j in 1:n, l in 1:m-k; A[l, j] = zero(T); end
            for j in 1:n
        ...
        for i in 1:k
            ...
            for jj in 1:t-1; A[r, jj] = conj(A[r, jj]); end  # ZLACGV essential
            A[r, t] = one(T)                                 # anchor
            τc = T <: Complex ? conj(tau[i]) : tau[i]        # forming Q applies H(i)ᴴ
            r > 1 && _corner_right!(view(A, 1:r-1, 1:t), view(A, r, 1:t), τc)
            for jj in 1:t-1; A[r, jj] *= -tau[i]; end         # DSCAL(−τ)
            for jj in 1:t-1; A[r, jj] = conj(A[r, jj]); end  # ZLACGV back
            A[r, t] = one(T) - τc
            for jj in t+1:n; A[r, jj] = zero(T); end
        end
    end
    return A
end
...
"""
function ormrq!(side::Char, trans::Char, A::AbstractMatrix{T}, tau::AbstractVector{T},
        C::AbstractMatrix{T}) where {T}
    m, n = size(C); k = length(tau)
    left = side == 'L'; notran = trans == 'N'
    nq = left ? m : n
    ...
    @inbounds begin
        ...
        while i3 > 0 ? i <= i2 : i >= i2
            ...
            for jj in 1:c-1; A[i, jj] = conj(A[i, jj]); end  # ZLACGV (no-op real)
            ...
            for jj in 1:c-1; A[i, jj] = conj(A[i, jj]); end  # ZLACGV back
            i += i3
        end
    end
    return C
