...
function geqp3!(A::AbstractMatrix{T}, jpvt::AbstractVector{<:Integer},
        tau::AbstractVector{T}) where {T<:BlasFloat}
    ...
    @inbounds for j in 1:n; jpvt[j] = j; end
    ...
    @inbounds for i in 1:k
        ...
        pvt = i; maxn = vn1[i]
        for j in i+1:n
            if vn1[j] > maxn; maxn = vn1[j]; pvt = j; end
        end
        if pvt != i
            for r in 1:m
