...
function run_checks()
    ...
    @printf("max err vs oracle = %.2e | max ON-vs-OFF (PB-vs-PB) = %.2e => %s\n",
            maxerr, maxpb, maxpb <= 1e-13 ? "FUSED==PACKED (equivalent)" : "DIVERGENT")
end
run_checks()
...
function fp_gf(k, n, fusedt)
    ...
    b = @be Bs (x -> (v=0.0; for B in x; v += f(B); end; v)) evals=1 samples=200 seconds=2.0
    P._TRSM_FUSEDT_ON[] = false
    median(getproperty.(b.samples, :time)) / length(Bs)
end
function gemm_gf(k, n)
    ...
    b = @be Bs (x -> (v=0.0; for B in x; v += g(B); end; v)) evals=1 samples=200 seconds=2.0
    2 * k^2 * n / (median(getproperty.(b.samples, :time)) / length(Bs)) / 1e9
end
...
for (k, n) in ((512,512),(512,256),(513,513),(640,640),(768,768),(896,896),(1024,1024))
    ...
    goff = n*k^2/toff/1e9; gon = n*k^2/ton/1e9
