...
function bench_full(k)
    ...
    for side in ('R', 'L')
        ...
        println(
            "  trmm! side=$side  k=$k   PB/OB = ", round(ratio(t_pb, t_ob), digits = 3),
            "   (pb=$(round(t_pb * 1.0e6, digits = 1))us ob=$(round(t_ob * 1.0e6, digits = 1))us)"
        )
    end
    return
end
...
function bench_R_paths(m, k)
    ...
    return println(
        "  side-R m=$m k=$k:  small(unpacked) PB/OB=", round(ratio(t_small, t_ob), digits = 3),
...
function bench_L_paths(n, k)
    ...
    return println(
        "  side-L n=$n k=$k:  small(unpacked) PB/OB=", round(ratio(t_small, t_ob), digits = 3),
        "   base(dense-gemm) PB/OB=", round(ratio(t_base, t_ob), digits = 3)
    )
end
...
function bench_gemm(m, n, k)
    ...
    return println("  gemm  m=$m n=$n k=$k   PB/OB=", round(ratio(t_pb, t_ob), digits = 3))
end
