...
bench(s, pad, reps, f, secs) = @be [mk(s,pad) for _ in 1:reps] (x -> (v=0.0; for c in x; v+=f(c); end; v)) evals=1 samples=40 seconds=secs
...
for s in SIZES
    ...
    @printf("n=%-5d  unpadded %.3f   padded %.3f   (Δ %+.1f%%)\n", s, median(r0), median(r1),
