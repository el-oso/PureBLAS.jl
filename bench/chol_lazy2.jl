...
function panel_clim!(A::AbstractMatrix{Float64}, N::Int, clim::Int)
    lda = stride(A, 2)
    Tb = _CHOL_T[]
    if size(Tb,1) < N+8; R = (N+8)%128==0 ? N+16 : N+8; Tb = _CHOL_T[] = Matrix{Float64}(undef, R, _CHOL_BLOCK); end
    ...
    GC.@preserve A Tb D begin
        ...
        while j < clim
            ...
            for c in 0:bs-1; unsafe_copyto!(pD + (c*ldD+c)*8, pjj + (c*lda+c)*8, bs-c); end
            _chol_rl_f64!(pD, bs, ldD, _CHOL_SB, _CHOL_STH) || throw(PosDefException(j+1))
            for c in 0:bs-1; unsafe_copyto!(pjj + (c*lda+c)*8, pD + (c*ldD+c)*8, bs-c); end
            ...
            if m > 0
                ...
                for c in 0:bs-1; unsafe_copyto!(p21 + c*lda*8, pT + c*ldT*8, m); end
                ...
                if cw > 0
                    ...
                    if cw * bs * 8 <= _L2_BYTES ÷ 2
                        ...
                        syrk!(view(A, j+bs+1:clim, j+bs+1:clim), view(Tb, 1:cw, 1:bs);
                              uplo='L', trans='N', alpha=-1.0, beta=1.0)
                    end
                    # below-clim rows [clim+1:N] × [j+bs+1:clim] → gemm (rank bs)
                    below = N - clim
                    if below > 0
                        gemm!(view(A, clim+1:N, j+bs+1:clim), view(Tb, cw+1:cw+below, 1:bs),
...
function chol_lazy2!(A, n::Int; NB2::Int=1024)
    ...
    while j < n
        ...
        if j > 0
            t = time_ns()
            syrk!(view(A, j+1:j+b, j+1:j+b), view(A, j+1:j+b, 1:j);
                  uplo='L', trans='N', alpha=-1.0, beta=1.0)
            if j + b < n
                gemm!(view(A, j+b+1:n, j+1:j+b), view(A, j+b+1:n, 1:j), view(A, j+1:j+b, 1:j);
