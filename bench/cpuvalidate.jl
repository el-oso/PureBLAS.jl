...
const DETECT = [
    (:_CPU_VENDOR, "vendor"), (:_SIMD_BYTES, "simd_B"), (:_L1_BYTES, "L1_B"), (:_L2_BYTES, "L2_B"),
    (:_L3_BYTES, "L3_B"), (:_CACHELINE, "line_B"), (:_INTEL_AVX2, "intelAVX2"),
]
const DERIVED = [
    (:_MR, "gemm_MR"), (:_NR, "gemm_NR"), (:_NC, "gemm_NC"), (:_KC, "gemm_KC"),
...
mktri(n) = (
    T = tril(randn(n, n)); @inbounds for i in 1:n
        T[i, i] += n
    end; T
)
...
ops = [
    (
        "gemm", n -> begin
            ...
            (
                () -> P.gemm!(C, A, Bm),
                () -> B.gemm!('N', 'N', 1.0, A, Bm, 0.0, C), 2.0 * n^3,
            )
        end,
    ),
    (
        "syrk", n -> begin
            A = randn(n, n); C = zeros(n, n)
            (
                () -> P.syrk!(C, A; uplo = 'L', trans = 'N', alpha = 1.0, beta = 0.0),
                () -> B.syrk!('L', 'N', 1.0, A, 0.0, C), 1.0 * n^3,
            )
        end,
    ),
    (
        "trsm", n -> begin
            A = mktri(n); B0 = randn(n, n); W = similar(B0)
            (
                () -> (copyto!(W, B0); P.trsm!(W, A; side = 'L', uplo = 'L', transA = 'N', diag = 'N', alpha = 1.0)),
                () -> (copyto!(W, B0); B.trsm!('L', 'L', 'N', 'N', 1.0, A, W)), 1.0 * n^3,
            )
        end,
    ),
    (
        "trmm", n -> begin
            ...
            (
                ...
                () -> (copyto!(W, B0); B.trmm!('L', 'L', 'N', 'N', 1.0, A, W)), 1.0 * n^3,
            )
        end,
    ),
    (
        "potrf", n -> begin
            S = mkspd(n); W = similar(S)
            (
                () -> (copyto!(W, S); P.potrf!(W; uplo = 'L')),
                () -> (copyto!(W, S); LinearAlgebra.LAPACK.potrf!('L', W)), n^3 / 3.0,
            )
        end,
    ),
]
...
sizes = let a = filter(x -> startswith(x, "sizes="), ARGS)
    isempty(a) ? [256, 512, 1024, 2048] : parse.(Int, split(split(a[1], "=")[2], ","))
end

host = gethostname()
cpu = try
    strip(read(pipeline(`lscpu`, `grep -m1 "Model name"`), String))
catch
    "?"
end
...
print(io, "#detect"); for (s, l) in DETECT
    print(io, "\t$l=$(_c(s))")
end; println(io)
print(io, "#derived"); for (s, l) in DERIVED
    print(io, "\t$l=$(_c(s))")
end; println(io)
...
for (nm, mk) in ops
    ...
    for n in sizes
        pb, ob, fl = mk(n)
        r, tp = ratio(pb, ob, n)
        push!(ratios, r); gflast = fl / tp / 1.0e9
    end
    append!(allr, ratios)
