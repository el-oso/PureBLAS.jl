...
function ab(m, n, k, rounds)
    ...
    for _ in 1:rounds
        ...
        t0 = time_ns(); for r in 1:reps
            B.gemm!('N', 'N', T(-1), A, Bm, T(1), Cob[r])
        end; t1 = time_ns()
        for r in 1:reps
            _gemm_3m!(false, false, false, false, m, n, k, T(-1), A, Bm, T(1), C3[r])
        end; t2 = time_ns()
        for r in 1:reps
            _gemm_cmplx_blocked!(false, false, false, false, m, n, k, T(-1), A, Bm, T(1), Cbl[r])
        end; t3 = time_ns()
        SINK[] += real(C3[1][1]) + real(Cbl[1][1]) + real(Cob[1][1])
        push!(r3, (t1 - t0) / (t2 - t1)); push!(rb, (t1 - t0) / (t3 - t2))
    end
    return (median(r3), median(rb))
end
# correctness sanity
let m = 64, n = 48, k = 40
