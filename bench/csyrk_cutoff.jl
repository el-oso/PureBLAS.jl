...
function med(f; rounds = 15)
    rs = Float64[]
    for _ in 1:rounds; c = f(); _run(c); t = time_ns(); c(); push!(rs, Float64(time_ns() - t)); end
    median(rs)
end
...
for (tc, tr) in (('N', false), ('C', true))
    for n in (8, 12, 16, 24, 32, 48)
        ...
        A = randn(T, n, n)
        scr = () -> zeros(T, n, n)
        ob = med(() -> (C = scr(); () -> (for _ in 1:r; B.herk!('U', tc, 1.0, A, 0.0, C); end)))
        pk = med(() -> (C = scr(); () -> (for _ in 1:r; P._syrk_scaleC!(C, true, 0.0); P._csyrk_packed!(true, tr, true, 1.0, A, C, n); end)))
        rc = med(() -> (C = scr(); () -> (for _ in 1:r; P._syrk_scaleC!(C, true, 0.0); P._syrk_rec!(true, tr, true, 1.0, A, C, n, P._l3_tmp(T), 0, n); end)))
