...
| const | file:line | value | hypothesis | Tier-2 plan |
|-------|-----------|-------|-----------|-------------|
| ⛔`_TRMM_RPACK` | level3.jl:14 | `448` (kept) | **DERIVATION FALSIFIED** — the "= gemm `_GEMM_UNPACK_MAX`" hypothesis regressed Zen3 (see below); reverted to literal 448 | Zen3 A/B: 96 slower 3–18% |
| ✅`_CHOL_RL_MAX` | lapack.jl:524 | `128:224` | **PARTIAL DERIVE** — AVX2 `√(_L2_BYTES/8)·7/8` (=224 Zen3); W=8 `128` halving base kept flat (µarch-invariant) | no-op all 3 boxes |
...
|-------|-----------|-------|------------------|
| `_SYRK_UNIFIED_MAX` | level3.jl:2875 | `W4? 48 : 0` | **INVARIANT-per-ISA (validated-by-gate).** µarch selector for the unified syrk path (AVX2 uses it ≤48; AVX-512 disables, `0`). Each side runs on its own µarch and **syrk gates ≥1.40× OB at every size on all 3 boxes** (Zen4 1.40 / Zen3 1.40 / Zen5 1.61 worst-median) → both sides gate-validated. Same class as `_GBMV_CONV_MAX`. Flipping the disabled AVX-512 side is a speculative "could-be-faster", not a fix (op already gates hard). |
