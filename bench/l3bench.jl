...
@inline _reps(k) = clamp(round(Int, TARGET_MS / (2 * k^3 / 50e9 * 1e3)), 1, 200)
...
function _stable(ob, our, reset; rounds = 9, tol = 0.02, cap = 45)
    ...
    m = median(rs); (m, (quantile(rs, 0.75) - quantile(rs, 0.25)) / m, length(rs))
end
...
function _variants(routine, k)
    ...
    if routine == "gemm"
        for (lab, ta, tb) in (("NN", Nn, Nn), ("NT", Nn, Tt), ("TN", Tt, Nn))
            our = () -> (for _ in 1:reps; PureBLAS.gemm!(C, A, Bm; transA = ta, transB = tb, alpha = 1.0, beta = 0.0); end; C[1])
            ob  = () -> (for _ in 1:reps; B.gemm!(ta, tb, 1.0, A, Bm, 0.0, C); end; C[1])
            ce  = () -> (C1 = copy(C); C2 = copy(C); B.gemm!(ta, tb, 1.0, A, Bm, 0.0, C1);
    ...
    elseif routine == "symm"
        for (lab, sd) in (("L", L), ("R", Rr))
            our = () -> (for _ in 1:reps; PureBLAS.symm!(C, A, Bm; side = sd, uplo = L, alpha = 1.0, beta = 0.0); end; C[1])
            ob  = () -> (for _ in 1:reps; B.symm!(sd, L, 1.0, A, Bm, 0.0, C); end; C[1])
            ce  = () -> (C1 = copy(C); C2 = copy(C); B.symm!(sd, L, 1.0, A, Bm, 0.0, C1);
    ...
    elseif routine == "syrk"
        for (lab, up, tr) in (("UN", U, Nn), ("LN", L, Nn), ("UT", U, Tt), ("LT", L, Tt))
            our = () -> (for _ in 1:reps; PureBLAS.syrk!(C, A; uplo = up, trans = tr, alpha = 1.0, beta = 0.0); end; C[1])
            ob  = () -> (for _ in 1:reps; B.syrk!(up, tr, 1.0, A, 0.0, C); end; C[1])
            ce  = () -> (C1 = copy(C); C2 = copy(C); B.syrk!(up, tr, 1.0, A, 0.0, C1);
                         PureBLAS.syrk!(C2, A; uplo = up, trans = tr, alpha = 1.0, beta = 0.0); _tri_relerr(C2, C1, up))
            push!(vs, (lab = lab, ob = ob, our = our, reset = nz, ce = ce))
        end
    elseif routine == "syr2k"
        for (lab, up, tr) in (("UN", U, Nn), ("LN", L, Nn), ("UT", U, Tt), ("LT", L, Tt))
            our = () -> (for _ in 1:reps; PureBLAS.syr2k!(C, A, Bm; uplo = up, trans = tr, alpha = 1.0, beta = 0.0); end; C[1])
            ob  = () -> (for _ in 1:reps; B.syr2k!(up, tr, 1.0, A, Bm, 0.0, C); end; C[1])
            ce  = () -> (C1 = copy(C); C2 = copy(C); B.syr2k!(up, tr, 1.0, A, Bm, 0.0, C1);
    ...
    elseif routine == "trmm"
        ...
        for (lab, up, tr) in (("UN", U, Nn), ("LN", L, Nn), ("UT", U, Tt), ("LT", L, Tt))
            ...
            ce  = () -> (B1 = copy(Bm); B2 = copy(Bm); B.trmm!(L, up, tr, Nd, 0.7, A, B1);
    ...
    elseif routine == "trsm"
        ...
        for (lab, up, tr) in (("UN", U, Nn), ("LN", L, Nn), ("UT", U, Tt), ("LT", L, Tt))
            ...
            ce  = () -> (B1 = copy(Bm); B2 = copy(Bm); B.trsm!(L, up, tr, Nd, 0.7, Ad, B1);
