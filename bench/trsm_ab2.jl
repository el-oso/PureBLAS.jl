# Same-process controlled A/B: fused gemmtrsm leaf vs current invL leaf, both vs ref (OB or AOCL).
# Toggles PureBLAS._TRSM_FUSED_ON[] within one process. Gate shape side=L uplo=U transA=N, square B.
# Usage: taskset -c 8 julia --project=bench bench/trsm_ab2.jl [aocl] [f32]
...
println("ref=$REF elt=$ELT   (PB_fused/ref  PB_invL/ref  per size)")
for s in SIZES
    reps = clamp(20_000_000 ÷ (s*s*s), 1, 512); secs = s >= 512 ? 2.0 : 2.5
    r_on = Float64[]; r_off = Float64[]
    for round in 1:3
        bo = bench(s, reps, ob1, secs)
        PureBLAS._TRSM_FUSED_ON[] = true;  bpn = bench(s, reps, pb1, secs)
        PureBLAS._TRSM_FUSED_ON[] = false; bpf = bench(s, reps, pb1, secs)
        push!(r_on, med(bo, bpn)); push!(r_off, med(bo, bpf))
    end
    @printf("n=%-5d  fused %.3f   invL %.3f   (Δ %+.1f%%)\n", s, median(r_on), median(r_off),
