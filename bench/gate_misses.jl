...
for (box,_) in BOXES
    data[box] = (ob = parse_cache("bench/plots_data_$(box).txt"),
                 aocl = parse_cache("bench/plots_data_$(box)_aocl.txt"))
end
# all (op,lvl,size) keys
allk = Set{Tuple{String,String,Int}}()
for (box,_) in BOXES, k in keys(data[box].ob); push!(allk, k); end
...
for op in sort(collect(keys(byop)), by=o->-length(byop[o]))
    @printf("\n## %s  (%d miss-cells)\n", op, length(byop[op]))
    for l in byop[op]; println(l); end
end
...
for (op,lvl,sz) in allk
    ...
    for (box,lab) in BOXES
        ...
        if w<1.0; worstall=min(worstall,w); push!(boxes,split(lab,'/')[1]); (!isnan(ob)&&ob<1.0)&&(obloss+=1); end
    end
    if worstall<1.0
