...
function sweep(mk,ob,pb,s;rounds=40)
    ...
    end; median(rs),std(rs)
end
a0=T(1.0000001,0.0)
xy(s)=(randn(T,s),randn(T,s)); x1(s)=(randn(T,s),)
println("=== zscal (real scalar a0) ===")
for s in (1000,3000,10000,30000)
    r=sweep(x1, (c,m)->(for _ in 1:m;B.scal!(a0,c[1]);end;c[1][1]), (c,m)->(for _ in 1:m;PureBLAS.scal!(a0,c[1]);end;c[1][1]), s)
    @printf("  n=%-6d %.3f (σ%.3f)\n", s, r...)
end
println("=== zdotc ===")
for s in (1000,3000,10000,30000,100000)
    r=sweep(xy, (c,m)->(s0=zero(T);for _ in 1:m;s0+=B.dotc(c[1],c[2]);end;s0), (c,m)->(s0=zero(T);for _ in 1:m;s0+=PureBLAS.dot(c[1],c[2]);end;s0), s)
    @printf("  n=%-6d %.3f (σ%.3f)\n", s, r...)
end
println("=== dzasum ===")
for s in (1000,3000,10000,30000,100000)
    r=sweep(x1, (c,m)->(s0=0.0;for _ in 1:m;s0+=B.asum(c[1]);end;s0), (c,m)->(s0=0.0;for _ in 1:m;s0+=PureBLAS.asum(c[1]);end;s0), s)
    @printf("  n=%-6d %.3f (σ%.3f)\n", s, r...)
end
