...
let ok = true, me = 0.0
    for uplo in ('U', 'L'), transA in ('N', 'T', 'C'), diag in ('N', 'U'), k in (8, 17, 64, 128), n in (16, 100)
        ...
        e > 1.0e-12 * k && (ok = false; @printf("CORRECTNESS FAIL uplo=%s tA=%s diag=%s k=%d n=%d err=%.2e\n", uplo, transA, diag, k, n, e))
    end
...
for (nm, obf, pbf) in (
    ...
    for n in sizes
        r = ratio(
            () -> (tri(n), rand(T, n, n), rep3(n)),   # fresh B per round; in-place reps (timing data-independent)
            c -> (
                for _ in 1:c[3]
                    obf(c[1], c[2])
                end; c[2][1]
