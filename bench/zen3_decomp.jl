...
for k in (32, 64, 128)
    ...
    tpk = med(
        () -> (
            C = randn(T, m, n); () -> (
                for _ in 1:reps
                    P.gemm!(C, A, Bm; alpha = one(T), beta = one(T))
                end; sig(C)
            )
        )
    )
    tup = med(
        () -> (
            C = randn(T, m, n); () -> (
                for _ in 1:reps
                    P._gemm_cmplx_unpacked_go!(false, false, m, n, k, one(T), A, Bm, one(T), C)
                end; sig(C)
            )
        )
    )
    @printf(
        "k=%-4d packed/OB=%.3f  unpacked/OB=%.3f  packed-is-%.2f×-faster-than-unpacked\n",
...
for k in (128, 256)
    ...
    tpb = med(
        () -> (
            Bc = copy(Bm); () -> (
                for _ in 1:reps
                    P.trmm!(Bc, A; side = 'L', uplo = 'U', transA = 'N', diag = 'N')
                end; sig(Bc)
            )
        )
    )
...
for k in (64, 128, 256)
    ...
    tkt = med(
        () -> (
            Bc = copy(Bm); () -> (
                for _ in 1:reps
                    P._trmm_cmplx_small_L!(true, false, false, false, k, A, Bc)
                end; sig(Bc)
            )
        )
    )
    tdn = med(
        () -> (
            Bc = copy(Bm); () -> (
                for _ in 1:reps
                    P._trmm_cmplx_base_L!(true, false, false, false, k, A, Bc)
                end; sig(Bc)
            )
        )
    )
    @printf(
        "k=%-4d K-TRIM=%.2fµs  dense=%.2fµs  dense/K-TRIM=%.2f×  (flop ratio ≈2×; <2× ⇒ overhead dominates)\n",
        k, tkt / reps / 1.0e3, tdn / reps / 1.0e3, tdn / tkt
    )
end
...
for n in (256, 512, 1024, 2048)
    ...
    tpb = med(
        () -> (
            yy = copy(y); () -> (
                for _ in 1:reps
                    P.gemv!(yy, A, x; alpha = one(T), beta = zero(T))
                end; real(yy[1])
            )
        )
    )
    tst = med(
        () -> () -> begin
            s = 0.0
            @inbounds @simd for i in eachindex(Ar)
                s += Ar[i]
            end
            s
        end
    ...
    @printf(
        "n=%-5d OB=%.0f  PB=%.0f  stream=%.0f GB/s   PB/OB=%.2f  PB/stream=%.2f  OB/stream=%.2f\n",
