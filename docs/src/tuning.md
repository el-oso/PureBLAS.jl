...
| `_double_pumped(hw)` | Full-width registers but **half-width FP datapath** (a 512-bit op occupies the 256-bit pipes twice). **Detected, not inferred**: AMD reports it in CPUID `Fn8000_001A` EAX (FP128/FP256/FP512). Was a `family == 0x19` lookup until 2026-09-09, when that proved wrong — family `0x1A` holds *both* native-512 parts (Granite Ridge, Turin) and double-pumped mobile ones (Strix, Krackan), and AMD firmware can switch the same Turin die between the two, so no identity table can express it. No leaf (Intel, ARM) ⇒ register width | `hw.fpw` vs `hw.simd` |
